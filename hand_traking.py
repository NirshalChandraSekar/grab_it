...
def sample_points_on_line(p1, p2, mask, n=20):
    """
...
# def sample_points_on_line(p1, p2, mask):
#     """
#     Sample points along the line segment between p1 and p2, extending in both directions
#     until a pixel outside the mask is encountered.

#     :param p1: Tuple (x1, y1) - Start point
#     :param p2: Tuple (x2, y2) - End point
#     :param mask: 2D array - Mask where non-zero values indicate valid regions
#     :return: List of (x, y) coordinates of valid points along the line
#     """
#     h, w = mask.shape[:2]
#     p1 = np.array(p1, dtype=float)
#     p2 = np.array(p2, dtype=float)
...
#     if np.allclose(d, 0):
#         # If p1 and p2 are the same point, return it if it's valid
#         contact_point = (int(round(p1[0])), int(round(p1[1])))
#         valid_points = [contact_point] if mask[contact_point[1], contact_point[0]] > 0 else []
#         return valid_points

#     # Normalize the direction vector
#     step_size = 1.0 / np.linalg.norm(d)
#     d_unit = d * step_size  # Unit direction vector

#     # Function to check if a point is inside the mask and image boundaries
#     def is_point_valid(x, y):
#         return 0 <= x < w and 0 <= y < h and mask[int(y), int(x)] > 0

#     # Extend the line in the direction of p1 to p2
#     current_point = p1.copy()
#     valid_points = []
#     while is_point_valid(current_point[0], current_point[1]):
#         valid_points.append((int(round(current_point[0])), int(round(current_point[1]))))
#         current_point += d_unit

#     # Extend the line in the opposite direction (p2 to p1)
#     current_point = p1 - d_unit  # Start just before p1
#     while is_point_valid(current_point[0], current_point[1]):
#         valid_points.append((int(round(current_point[0])), int(round(current_point[1]))))
#         current_point -= d_unit
